<?php
  require_once("db_.php");

	$idrel=$_REQUEST['idrel'];
  $pd = $db->cliente_editar($idrel);
  $correo=$pd->correo;
  $pass=$pd->pass;

?>
<form is="f-submit" id="form_pass" db="a_pacientes/db_" fun="password" cmodal="1">
	<div class='modal-header'>
		<h5 class='modal-title'>Accesos</h5>
	</div>
  <div class='modal-body' >

		<label class='input_title'>Usuario:</label>
		<div class='form-group input-group'>
			<div class='input-group-prepend'>
				<span class='input-group-text'> <i class='fas fa-user-circle'></i>
			</div>
			<input class='form-control' placeholder='Contraseña' type='text' value='<?php echo $correo;?>' id='pass1' name='pass1' readonly>
		</div>

		<label class='input_title'>Contraseña:</label>
		<div class='form-group input-group'>
			<div class='input-group-prepend'>
				<span class='input-group-text'> <i class='fa fa-lock'></i>
			</div>
			<input class='form-control' placeholder='Repetir contraseña' type='text'  value='<?php echo $pass;?>' id='pass2' name='pass2' readonly>
		</div>
	</div>
	<div class="modal-footer">
		<button type="button" class="btn btn-warning" data-dismiss="modal" title='Cancelar'>Cerrar</button>
  </div>
</form>
