<?php
  ...
  for($i = $begin; $i <= $end; $i->modify('+1 day')){
		...
		//echo "------------>".$i->format("w");
			$dia_sem=$i->format("w");
	    $sql="select * from consultorio_horarios where idconsultorio=$idconsultorio and hasta_dia=$dia_sem";
			    ...
			    foreach($disponibles as $key){
						...
						echo "<div class='body-row' >";
							...
							echo "</div>";
								...
								$sql="select clientes.nombre,clientes.apellidop, clientes.apellidom, citas.estatus from citas left outer join clientes on clientes.id=citas.idpaciente where citas.idconsultorio=$idconsultorio and citas.desde='$dia $hora'";
