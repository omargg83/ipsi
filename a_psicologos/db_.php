...
class Psic extends ipsi{
	...
	public function psicologos_lista(){
		try{
			self::set_names();
	...
	public function psicologo_editar($id){
		try{
		  self::set_names();
		  $sql="select * from psicologos where idpsicologo=:id";
	...
	public function guardar_psicologo(){
		$x="";
		parent::set_names();
		$arreglo =array();
		$id=$_REQUEST['id'];
		...
		if($id==0){
			$x=$this->insert('psicologos', $arreglo);
		}
		else{
			$x=$this->update('psicologos',array('idpsicologo'=>$id), $arreglo);
		}
		return $x;
	}
